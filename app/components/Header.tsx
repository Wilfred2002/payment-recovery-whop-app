...
export default function Header({ showNav = false, companyId }: HeaderProps) {
	...
	return (
		<header className="px-4 sm:px-8 py-4 border-b border-mint-100 dark:border-[var(--color-dark-border)] bg-white dark:bg-[var(--color-dark-bg)]">
			<div className="flex items-center justify-between">
				<h1 className="text-2xl sm:text-4xl font-bold text-mint-700 dark:text-[var(--color-dark-text)] font-[family-name:var(--font-space-mono)] uppercase tracking-tight">
					Rebound
				</h1>
						...
						<Link
							...
							className={`text-sm font-semibold font-[family-name:var(--font-space-mono)] uppercase tracking-wider transition-colors ${
								isActive("/dashboard") || isActive("/discover/dashboard")
									? "text-mint-700 dark:text-[var(--color-dark-text)] border-b-2 border-mint-700 dark:border-neutral-400"
									: "text-mint-600 dark:text-[var(--color-dark-text)] hover:text-mint-700 dark:hover:text-[var(--color-dark-text)]"
							}`}
						>
							Dashboard
						</Link>
